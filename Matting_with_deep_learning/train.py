...
if is_train:
    with tf.Session(config=config) as sess:
        ...
        for idx in range(10000):
            ...
            print sess.run(tf.get_default_graph().get_tensor_by_name("Outer/fc13/x:0"),
                feed_dict={F:F_train, B:B_train, I:I_train, alpha_diff:alpha_diff_target})
            for v in (zip(alpha_diff_target, sess.run(tf.get_default_graph().get_tensor_by_name("Outer/fc13/x:0"),
                feed_dict={F:F_train, B:B_train, I:I_train, alpha_diff:alpha_diff_target}))):
    ...
    with tf.Session(config=config) as sess:
        ...
        print(sess.run(tf.get_default_graph().get_tensor_by_name("Outer/fc13/x:0"), feed_dict={F:F_train, B:B_train, I:I_train, alpha_diff:alpha_diff_target}))
