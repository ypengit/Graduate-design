...
if is_train:
    with tf.Session(config=config) as sess:
        ...
        for idx_f in range(100):
            pics = np.load("/disk3/data/{:0>3}.npy".format(idx_f))
            for ix in range(0,10000,batch_size):
                ...
                if ix % 1000 == 0:
                    learning_rate *= 0.985
                if ix % 200 == 0:
                    print 'the idx is %05d'% (ix+idx_f*10000), 'before',sess.run(loss, feed_dict={F:F_train, B:B_train, I:I_train, alpha_diff:alpha_diff_target})
